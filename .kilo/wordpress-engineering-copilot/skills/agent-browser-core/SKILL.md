...
description: "use for ordinary website and webapp automation with agent-browser: opening pages, snapshots, clicking, filling, typing, selecting, uploading, extracting visible data, screenshots, tabs, waits, auth state, and troubleshooting stale refs. trigger for browser tasks in chatgpt, codex, openai agents sdk, vs code, jetbrains, and local model-agnostic agents."
---

# Model-agnostic adaptation

This skill is adapted from the upstream agent-browser skill corpus for ChatGPT, Codex, OpenAI Agents SDK, VS Code, JetBrains, and local/open-source agent runtimes. Keep provider-specific assumptions out of task execution unless the user explicitly asks for a provider-specific path.
...
- **Electron desktop app** (VS Code, Slack desktop, Discord, Figma, etc.):
  `Use the `agent-browser-electron` skill`
- **Slack workspace automation**: `Use the `agent-browser-slack` skill`
- **Exploratory testing / QA / bug hunts**: `Use the `agent-browser-dogfood` skill`
- **Vercel Sandbox microVMs**: `Use the `agent-browser-vercel-sandbox` skill`
- **AWS Bedrock AgentCore cloud browser**: `Use the `agent-browser-agentcore` skill`
...
```bash
Use the `agent-browser-core` skill for the command loop and reference model --full
```
